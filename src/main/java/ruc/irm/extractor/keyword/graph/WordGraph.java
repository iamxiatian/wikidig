...
//        }
        ...
        for (int i = 0; i < g.labels.length && count < topN; i++) {
            ...
            if (!ChineseStopKeywords.isStopKeyword(word)) {
                //判断与邻居节点是否构成了短语
                //System.out.println(word + ":" + getNodeScore(word));
                keywords.add(word);
                count++;
            }
        }
        ...
        for (int i = 0; i < sortedEdges.size() && i < topN; i++) {
        ...
        for (int i = 0; i < keywords.size(); i++) {
            ...
            for (int j = 0; j < biPhrases.size(); j++) {
                ...
                if (p.src.equals(keyword) || p.dest.equals(keyword)) {
                    score += 2 * p.score; //分值加上所在短语的分值（分值越大，越不重要）
                }
            }
            phrase.score = score;
        ...
        return results.stream().map(p -> p.getLabel()).collect(Collectors.toList());
    }
